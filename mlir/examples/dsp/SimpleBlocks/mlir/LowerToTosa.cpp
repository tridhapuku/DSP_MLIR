//====- LowerToTosaLoops.cpp - Partial lowering from Toy to Tosa +Std --===//
//
...
//===----------------------------------------------------------------------===//
// ToyToAffine RewritePatterns: FFT2D operations
//===----------------------------------------------------------------------===//
struct FFT2dOpLowering: public ConversionPattern {
      FFT2dOpLowering(MLIRContext *ctx)
        : ConversionPattern(dsp::fft2dOp::getOperationName(), 1 , ctx) {}
...
namespace {
struct ToyToTosaLoweringPass
    : public PassWrapper<ToyToTosaLoweringPass, OperationPass<ModuleOp>> {
  MLIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(ToyToTosaLoweringPass)
...
void ToyToTosaLoweringPass::runOnOperation() {
...
std::unique_ptr<Pass> mlir::dsp::createLowerToTosaPass() {
  return std::make_unique<ToyToTosaLoweringPass>();
}
